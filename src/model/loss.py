...
class InfoNCEWithExtraNeg(nn.Module):
    def __init__(self, temperature, similarity_type):
        super(InfoNCEWithExtraNeg, self).__init__()
        self.temperature = temperature  # temperature
        self.sim_type = similarity_type  # cos or dot
        self.criterion = nn.CrossEntropyLoss()

    def forward(self, pos_view_1, pos_view_2, extra_neg=None):
        """
        Args:
        ...
        Returns: nce_loss (FloatTensor, (,)): calculated nce loss
        """
        if pos_view_1.ndim > 2:
            # flatten tensor
            pos_view_1 = pos_view_1.view(pos_view_1.size(0), -1)
            pos_view_2 = pos_view_2.view(pos_view_2.size(0), -1)
            if extra_neg is not None:
                extra_neg = extra_neg.view(extra_neg.size(0), -1)

        if self.sim_type not in ['cos', 'dot']:
            raise Exception(f"Invalid similarity_type for cs loss: [current:{self.sim_type}]. "
                            f"Please choose from ['cos', 'dot']")
        sim13, sim23 = None, None
        if self.sim_type == 'cos':
            sim11 = self.cosinesim(pos_view_1, pos_view_1)
            sim22 = self.cosinesim(pos_view_2, pos_view_2)
            sim12 = self.cosinesim(pos_view_1, pos_view_2)
            if extra_neg is not None:
                sim13 = self.cosinesim(pos_view_1, extra_neg)
                sim23 = self.cosinesim(pos_view_2, extra_neg)
        else:  # dot
            # calc similarity
            sim11 = pos_view_1 @ pos_view_1.t()
            sim22 = pos_view_2 @ pos_view_2.t()
            sim12 = pos_view_1 @ pos_view_2.t()
            if extra_neg is not None:
                sim13 = pos_view_1 @ extra_neg.t()
                sim23 = pos_view_2 @ extra_neg.t()
        # mask non-calc value
        sim11[..., range(sim11.size(0)), range(sim11.size(0))] = float('-inf')
        sim22[..., range(sim22.size(0)), range(sim22.size(0))] = float('-inf')
        ...
        cl_logits1 = torch.cat([sim12, sim11], -1)
        if extra_neg is not None:
            cl_logits1 = torch.cat([cl_logits1, sim13], -1)
        cl_logits2 = torch.cat([sim22, sim12.t()], -1)
        if extra_neg is not None:
            cl_logits2 = torch.cat([cl_logits2, sim23], -1)
        cl_logits = torch.cat([cl_logits1, cl_logits2], 0) / self.temperature

        target = torch.arange(cl_logits.size(0)).long().to(pos_view_1.device)
        cl_loss = self.criterion(cl_logits, target)
