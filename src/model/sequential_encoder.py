...
class Transformer(nn.Module):
    ...
    def forward(self, item_input, seq_embedding, output_all_encoded_layers=False, out_attn_score=False):
        """
        ...
        for layer in self.encoder_layers:
            seq_embedding, attn_score = layer(seq_embedding, mask)
            all_encoded_layers.append(seq_embedding)
            all_attn_scores.append(attn_score)

        if not output_all_encoded_layers:
            all_encoded_layers = all_encoded_layers[-1]
        if not out_attn_score:
            return all_encoded_layers
        return all_encoded_layers, all_attn_scores
...
class MultiHeadAttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, inputs_mask=None):
        ...
        :return: out: [B, max_len, embed_size] out_attn_score: [B, max_len, max_len]
        """
        ...
        attention_score = F.softmax(energy, dim=-1)  # [B, H, L, L]
        dropped_attn_score = self.attn_dropout(attention_score)

        out = torch.matmul(dropped_attn_score, V)  # [batch_size, n_head, max_len, head_dim]
        out = out.permute((0, 2, 1, 3)).contiguous()  # memory layout
        out = out.view((batch_size, -1, self.embed_size))
        out = self.fc_o(out)

        out_attn_score = attention_score.mean(dim=1)
        return out, out_attn_score
